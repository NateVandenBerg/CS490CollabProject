...
$requiredKeys = ['formID', 'firstName', 'lastName', 'column1'];
...
$query = "INSERT INTO " . $tableName . " (first_name, last_name, title, email, description, active) VALUES (?, ?, ?, ?, ?, 1)";
$stmt = $conn->prepare($query);
if (!$stmt) {
    echo json_encode(['error' => 'Failed to prepare statement...create_collaborator.php']);
    exit;
}
$stmt->bind_param("sssss",$firstName,$lastName, $nameORtitle, $email, $desc);
$result = $stmt->execute();
